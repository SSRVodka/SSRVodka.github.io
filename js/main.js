...
//获取天气
// 高德开发者 Key
let key = "21dd233da1915b6df3f222df71d59857"; // key
  ...
  // 获取地理位置信息
    ...
    .then((res) => {
      if (res.status) {
        ...
        let curAdCode = res.adcode;
        // 获取天气信息
        fetch(
        `https://restapi.amap.com/v3/weather/weatherInfo?key=${key}&city=${curAdCode}`
         )
        .then(response => response.json())
        .then((res) => {
            if (res.status) {
              ...
              $('#win_text').html(res.lives[0].winddirection);
...
$('#switchmore').on('click', function () {
    ...
    if (shoemore && $(document).width() >= 990) {
        ...
        $("#change1").html("Do one thing at a time, and do well.");
    }
});
